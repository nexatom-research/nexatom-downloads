...
| `nexatom_tt_set_channel_input_delay` | `[In] nexatom_tt_handle device`<br>`[In] uint8_t channel`<br>`[In] uint32_t delay_ps` | `nexatom_error_code_t` | Requests delay in ps from 0 to the resolved profile's `max_channel_input_delay_ps`, which never exceeds `NEXATOM_MAX_CHANNEL_INPUT_DELAY_PS` (256000 ps). Current bootloader-based profiles allow the full 256000 ps; use the resolved limit, since older hardware can report less (the old universal 4000 ps assumption no longer applies). A larger value returns `NEXATOM_ERROR_INVALID_PARAMETER`. The call is legal while acquiring: rewriting the same value changes nothing, while a real change is fenced in hardware and can drop events in flight. |
