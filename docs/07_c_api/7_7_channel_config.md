...
| `nexatom_tt_set_channel_input_delay` | `[In] nexatom_tt_handle device`<br>`[In] uint8_t channel`<br>`[In] uint32_t delay_ps` | `nexatom_error_code_t` | Requests delay in ps up to the resolved `max_channel_input_delay_ps`; it is not restricted to the older universal 4000 ps assumption. |
