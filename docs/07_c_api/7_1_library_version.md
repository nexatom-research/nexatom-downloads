...
| `nexatom_tt_get_library_info` | `[Out] char* info_buffer`<br>`[In] size_t buffer_size` | `nexatom_error_code_t` | Extracts a deeply detailed build string (including compiler version, architecture, and Git commit hash) into the user-allocated `info_buffer`. |
...
| `nexatom_tt_get_last_error_message` | *None* | `const char*` | Returns the detailed, internal C++ exception text from the most recent API failure on the calling thread. The pointer remains valid until the next API call. |

### Error Handling Example (C/C++)

```c
nexatom_tt_handle device = NULL;
nexatom_error_code_t status = nexatom_tt_connect(device, 5000);
