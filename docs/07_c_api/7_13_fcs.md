...
| `nexatom_tt_set_fcs_experimental_conditions`| `[In] nexatom_tt_handle device`<br>`[In] double temperature_celsius`<br>`[In] double viscosity_mPa_s`<br>`[In] double wavelength_nm`<br>`[In] double calibration_diffusion_um2_s`| `nexatom_error_code_t` | Feeds the solver ambient conditions and an optional `calibration_diffusion_um2_s` reference value to automatically adjust the confocal volume parameters based on a known standard (e.g., Rhodamine 6G). |
| `nexatom_tt_set_fcs_fitting_control`| `[In] nexatom_tt_handle device`<br>`[In] double tolerance`<br>`[In] int max_iterations`<br>`[In] double initial_n`<br>`[In] double initial_tau_d` | `nexatom_error_code_t` | Seeds the fitter with starting estimates for the number of particles (N) and diffusion time (τD) to ensure mathematical convergence. |

### Data Structures: `nexatom_fcs_analysis_result_t`

When FCS is enabled, the `nexatom_corm_callback_data_t` payload (dispatched via the callback registered in Section 7.8) will populate its nested `analysis_result.fcs` struct. Note that general fit quality (e.g., χ²) is still located in the parent `fit_result` struct.
