...
When DCS is enabled, the `nexatom_corm_callback_data_t` payload (dispatched via the callback registered in Section 7.8) will populate its nested `analysis_result.dcs` struct. Note that general fit quality (e.g., χ²) and the β coherence factor are still located in the parent `fit_result` struct.
