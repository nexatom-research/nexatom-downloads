...
1.  **Image Validation:** Supply a separately qualified `.bin` image for the target instrument. The loader accepts `<NAME>_<VERSION>.bin`, where `NAME` is four ASCII alphanumeric characters and `VERSION` is an unsigned decimal integer. Use the current image from the [firmware catalogue](../01_getting_started/1_4_firmware.md#firmware-catalogues): `Z080_nnn.bin` for a UTT810, `K168_nnn.bin` for a UTT160810. No image is bundled with the SDK. Verify the catalogue checksum and target compatibility; there is no generic extended-filename/binary-header fallback in this workflow.
