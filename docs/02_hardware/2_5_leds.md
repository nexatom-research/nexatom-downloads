...
The 32-bit `mode` parameter selects the hardware-defined LED mode. There is no common public color/blink lookup table. In Python the equivalent method is `device.set_rgb_led_mode(mode)`; use a value documented for your supplied image rather than guessing one.
